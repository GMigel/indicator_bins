...
void app_run() {
  ...
  while (1) {
    // ---------- read encoder input ----------
    int delta = input_get_enc() - prev_enc;
    prev_enc += delta;

    encoder.serve_input(delta, input_get_btn(), systime);
...
// CAN processing
void can_serve() {
  static std::array<uint32_t, CAN_TOTAL_BUFNR> rx_time{}; // zeros
  static std::array<float, CAN_TOTAL_BUFNR> rx_val{};
  enum { ATTITUDE_TIMEOUT = 100, AIR_TIMEOUT = 500 };

  for (int i = 0; i < CAN_TOTAL_BUFNR; i++) {
    FDCAN_RxHeaderTypeDef rx_hdr;
    uint8_t buf[8];

    if (HAL_FDCAN_IsRxBufferMessageAvailable(&can, i) &&
        HAL_FDCAN_GetRxMessage(&can, i, &rx_hdr, buf) == HAL_OK) {
  ...
  if (((systime - rx_time[CAN_BUFNR_ROLL1]) < ATTITUDE_TIMEOUT) ||
    ...
    gui_state.roll =
        rx_val[((systime - rx_time[CAN_BUFNR_ROLL1]) < ATTITUDE_TIMEOUT) ? CAN_BUFNR_ROLL1
                                                                         : CAN_BUFNR_ROLL2] /
        180.0f * PI;
  } else
  ...
  if (((systime - rx_time[CAN_BUFNR_PITCH1]) < ATTITUDE_TIMEOUT) ||
    ...
    gui_state.pitch =
        rx_val[((systime - rx_time[CAN_BUFNR_PITCH1]) < ATTITUDE_TIMEOUT) ? CAN_BUFNR_PITCH1
                                                                          : CAN_BUFNR_PITCH2] /
        180.0f * PI;
  } else
  ...
  if (((systime - rx_time[CAN_BUFNR_HEADING1]) < ATTITUDE_TIMEOUT) ||
    ...
    gui_state.heading =
        rx_val[((systime - rx_time[CAN_BUFNR_HEADING1]) < ATTITUDE_TIMEOUT) ? CAN_BUFNR_HEADING1
                                                                            : CAN_BUFNR_HEADING2] /
        180.0f * PI;
  } else
  ...
  if (((systime - rx_time[CAN_BUFNR_PRES1]) < AIR_TIMEOUT) ||
    ...
    float pres = rx_val[((systime - rx_time[CAN_BUFNR_PRES1]) < AIR_TIMEOUT) ? CAN_BUFNR_PRES1
                                                                             : CAN_BUFNR_PRES2];
  ...
  if (((systime - rx_time[CAN_BUFNR_AIRSPEED1]) < AIR_TIMEOUT) ||
    ...
    gui_state.speed =
        rx_val[((systime - rx_time[CAN_BUFNR_AIRSPEED1]) < AIR_TIMEOUT) ? CAN_BUFNR_AIRSPEED1
                                                                        : CAN_BUFNR_AIRSPEED2] *
        3.6f;
  } else
  ...
  if (((systime - rx_time[CAN_BUFNR_VERTSPEED1]) < AIR_TIMEOUT) ||
    ...
    gui_state.vert_speed =
        rx_val[((systime - rx_time[CAN_BUFNR_VERTSPEED1]) < AIR_TIMEOUT) ? CAN_BUFNR_VERTSPEED1
