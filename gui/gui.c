...
static uint8_t cvs_buf_background[LV_CANVAS_BUF_SIZE_TRUE_COLOR(MAX_X, MAX_Y)]
    __attribute__((section(".lvgl_ram")));
static uint8_t cvs_buf_foreground[LV_CANVAS_BUF_SIZE_TRUE_COLOR_ALPHA(FOREGROUND_W, FOREGROUND_H)]
    __attribute__((section(".lvgl_ram")));
static uint8_t cvs_buf_hdng_panel[LV_CANVAS_BUF_SIZE_TRUE_COLOR(PANEL_HDNG_W, PANEL_HDNG_H)]
    __attribute__((section(".lvgl_ram")));
static uint8_t cvs_buf_hdng_scale[LV_CANVAS_BUF_SIZE_TRUE_COLOR_ALPHA(SCALE_HDNG_W, SCALE_HDNG_H)]
    __attribute__((section(".lvgl_ram")));
static uint8_t cvs_buf_speed_v[LV_CANVAS_BUF_SIZE_TRUE_COLOR(PANEL_VSPD_W, PANEL_VSPD_H)]
    __attribute__((section(".lvgl_ram")));
static uint8_t cvs_buf_speed[LV_CANVAS_BUF_SIZE_TRUE_COLOR_ALPHA(PANEL_SPD_W, PANEL_SPD_H)]
    __attribute__((section(".lvgl_ram")));
static uint8_t cvs_buf_plane[LV_CANVAS_BUF_SIZE_TRUE_COLOR_ALPHA(PLANE_W, PLANE_H)]
    __attribute__((section(".lvgl_ram")));
static uint8_t cvs_buf_pres[LV_CANVAS_BUF_SIZE_TRUE_COLOR(PANEL_PRES_W, PANEL_PRES_H)]
    __attribute__((section(".lvgl_ram")));
static uint8_t cvs_buf_alt[LV_CANVAS_BUF_SIZE_TRUE_COLOR_ALPHA(PANEL_ALT_W, PANEL_ALT_H)]
    __attribute__((section(".lvgl_ram")));
static uint8_t cvs_buf_ref[LV_CANVAS_BUF_SIZE_TRUE_COLOR_ALPHA(PANEL_REF_W, PANEL_REF_H)]
    __attribute__((section(".lvgl_ram")));
static uint8_t cvs_buf_msg_box[LV_CANVAS_BUF_SIZE_TRUE_COLOR(PANEL_MSG_W, PANEL_MSG_H)]
    __attribute__((section(".lvgl_ram")));
...
void gui_init(gui_state_t *state) {
  ...
  lv_canvas_set_buffer(cvs_msg_box, cvs_buf_msg_box, PANEL_MSG_W, PANEL_MSG_H,
                       LV_IMG_CF_TRUE_COLOR);
...
void init_cvs_speed_v(void) {
  ...
  const lv_point_t border[] = {{0, 0},
                               {PANEL_VSPD_W - 1, 0},
                               {PANEL_VSPD_W - 1, PANEL_VSPD_H - 1},
                               {0, PANEL_VSPD_H - 1},
                               {0, 0}};
...
void init_cvs_speed(void) {
  ...
  const lv_point_t border[] = {{0, Y0},       {7, Y0 - 5},   {7, Y0 - 12},  {32, Y0 - 12},
                               {32, Y0 - 26}, {49, Y0 - 26}, {49, Y0 + 25}, {32, Y0 + 25},
                               {32, Y0 + 11}, {7, Y0 + 11},  {7, Y0 + 4},   {0, Y0}};
...
void init_cvs_plane(void) {
  ...
  const lv_point_t triang_left[] = {{X0 - ROLL_SCALE_R + 7, Y0},
                                    {X0 - ROLL_SCALE_R + 13, Y0 - 3},
                                    {X0 - ROLL_SCALE_R + 13, Y0 + 3},
                                    {X0 - ROLL_SCALE_R + 7, Y0}};
  const lv_point_t triang_right[] = {{X0 + ROLL_SCALE_R - 7, Y0},
                                     {X0 + ROLL_SCALE_R - 13, Y0 - 3},
                                     {X0 + ROLL_SCALE_R - 13, Y0 + 3},
                                     {X0 + ROLL_SCALE_R - 7, Y0}};
...
void init_cvs_alt(void) {
  ...
  const lv_point_t border[] = {{0, Y0 - 12},  {27, Y0 - 12}, {27, Y0 - 20}, {49, Y0 - 20},
                               {49, Y0 - 5},  {55, Y0},      {49, Y0 + 4},  {49, Y0 + 19},
                               {27, Y0 + 19}, {27, Y0 + 11}, {0, Y0 + 11},  {0, Y0 - 12}};
...
void draw_foreground(float pitch, float roll, uint8_t is_pitch_valid, uint8_t is_roll_valid) {
  ...
  if (is_pitch_valid) {
    ...
    const float pitch_deg =
        is_pitch_valid ? MODF(pitch * 180.0f / PI + 90.0f, 180.0f) - 90.0f : 0.0f;
    ...
    for (int i = -3; i <= 3; i++) {
      ...
      if (deg_lbl) {
        ...
        lv_canvas_draw_text(cvs_foreground, X0 - PitchW / 2, Y0 + dy - i * LineStep - 7, PitchW,
                            &label, text);

        line.width = 2;
        lv_point_t line_left[] = {{X0 - 32, Y0 + dy - i * LineStep},
                                  {X0 - 15, Y0 + dy - i * LineStep}};
        lv_canvas_draw_line(cvs_foreground, line_left, 2, &line);
        lv_point_t line_right[] = {{X0 + 32, Y0 + dy - i * LineStep},
                                   {X0 + 15, Y0 + dy - i * LineStep}};
        lv_canvas_draw_line(cvs_foreground, line_right, 2, &line);
      } else {
        line.width = 2;
        lv_point_t line_up[] = {{X0 - 10, Y0 + dy - i * LineStep - 8},
                                {X0 + 10, Y0 + dy - i * LineStep - 8}};
        lv_canvas_draw_line(cvs_foreground, line_up, 2, &line);
        lv_point_t line_down[] = {{X0 - 10, Y0 + dy - i * LineStep + 7},
                                  {X0 + 10, Y0 + dy - i * LineStep + 7}};
        lv_canvas_draw_line(cvs_foreground, line_down, 2, &line);

        line.width = 1;
        lv_point_t line_middle[] = {{X0 - 15, Y0 + dy - i * LineStep},
                                    {X0 + 15, Y0 + dy - i * LineStep}};
        lv_canvas_draw_line(cvs_foreground, line_middle, 2, &line);
      }

      line.width = 1;
      lv_point_t line_tick[] = {{X0 - 15, Y0 + dy - i * LineStep - 15},
                                {X0 + 15, Y0 + dy - i * LineStep - 15}};
  ...
  for (int i = 0; i <= 180; i += 5) {
    ...
    float sc = PI / 180.0f;
    line.width = 1;
    lv_point_t line_tick[] = {
        {X0 + roundf((R - 5.0f) * cosf(i * sc)), Y0 + roundf((R - 5.0f) * sinf(i * sc))},
  ...
  for (int i = 0; i <= 300; i += 15) {
    ...
    line.width = 2;
    lv_point_t line_tick[] = {
        {X0 + roundf((R - 7.0f) * cosf(i * sc)), Y0 + roundf((R - 7.0f) * sinf(i * sc))},
        {X0 + roundf(R * cosf(i * sc)), Y0 + roundf(R * sinf(i * sc))}};
    lv_canvas_draw_line(cvs_foreground, line_tick, 2, &line);
  ...
  static const lv_point_t triang_down[] = {
      {X0, Y0 + R}, {X0 - 3, Y0 + R - 6}, {X0 + 3, Y0 + R - 6}, {X0, Y0 + R}};
  static const lv_point_t triang_up[] = {
      {X0, Y0 - R}, {X0 - 3, Y0 - R + 6}, {X0 + 3, Y0 - R + 6}, {X0, Y0 - R}};
  ...
  if (is_roll_valid) {
    const float roll_deg =
        is_roll_valid ? MODF(roll * 180.0f / PI + 180.0f, 360.0f) - 180.0f : 0.0f;
...
void draw_background(float pitch, float roll, uint8_t is_pitch_valid, uint8_t is_roll_valid) {
  ...
  static const lv_point_t triang_down[] = {{X0, Y0 + ROLL_SCALE_R},
                                           {X0 - 3, Y0 + ROLL_SCALE_R + 6},
                                           {X0 + 3, Y0 + ROLL_SCALE_R + 6},
                                           {X0, Y0 + ROLL_SCALE_R}};
  static const lv_point_t triang_up[] = {{X0, Y0 - ROLL_SCALE_R},
                                         {X0 - 3, Y0 - ROLL_SCALE_R - 6},
                                         {X0 + 3, Y0 - ROLL_SCALE_R - 6},
                                         {X0, Y0 - ROLL_SCALE_R}};
  rect.bg_color = COLOR_WHITE;
...
void draw_heading(float heading, uint8_t is_heading_valid, uint8_t is_icc) {
  ...
  static const lv_point_t triang[] = {{SCALE_HDNG_W / 2 - 3, 2},
                                      {SCALE_HDNG_W / 2 + 3, 2},
                                      {SCALE_HDNG_W / 2, 9},
                                      {SCALE_HDNG_W / 2 - 3, 2}};
  rect.bg_color = COLOR_WHITE;
  ...
  static const lv_point_t frame[] = {
      {1, SCALE_HDNG_H}, {1, 1}, {SCALE_HDNG_W - 1, 1}, {SCALE_HDNG_W - 1, SCALE_HDNG_H}};
...
void draw_pressure(float pressure, gui_mode_t mode, gui_unit_pres_t unit) {
  ...
  if (mode == GUI_SET_PRES_VAL) {
    const lv_point_t border[] = {{1, 1},
                                 {PANEL_PRES_W - 1, 1},
                                 {PANEL_PRES_W - 1, PANEL_PRES_H - 1},
                                 {1, PANEL_PRES_H - 1},
                                 {1, 1}};
  ...
  label.align = LV_TEXT_ALIGN_CENTER;
  label.font = &lv_font_sans18;
  lv_canvas_draw_text(cvs_pres, (PANEL_PRES_W - TextW) / 2, Y0 - TextH / 2 + 2, TextW, &label,
                      text);
}
//------------------------------------------------------------------------------
...
void draw_pres_ref(gui_type_pres_t type, gui_mode_t mode) {
  ...
  switch (type) {
  case GUI_QFE:
    lv_canvas_draw_text(cvs_ref, (PANEL_REF_W - TextW) / 2, Y0 - TextH / 2 + 2, TextW, &label,
                        "QFE");
    break;
  case GUI_QNH:
    lv_canvas_draw_text(cvs_ref, (PANEL_REF_W - TextW) / 2, Y0 - TextH / 2 + 2, TextW, &label,
...
void draw_alt_scale(float altitude, uint8_t is_alt_valid, gui_unit_alt_t unit) {
  ...
  lv_canvas_draw_rect(cvs_background, MAX_X - SIDE_PANEL_W, Y0 + 25, SIDE_PANEL_W,
                      MAX_Y - (Y0 + 25), &rect);
  ...
  for (int i = -4; i <= 3; i++) {
    line.color = COLOR_WHITE;
    line.width = 2;
    lv_point_t line_maj[] = {{MAX_X - LineW, Y0 + dy - i * LineStep},
                             {MAX_X, Y0 + dy - i * LineStep}};
    lv_canvas_draw_line(cvs_background, line_maj, 2, &line);
    lv_point_t line_min[] = {{MAX_X - LineW / 2, Y0 + dy - i * LineStep + LineStep / 2},
                             {MAX_X, Y0 + dy - i * LineStep + LineStep / 2}};
    ...
    label.font = &lv_font_sans14;
    lv_canvas_draw_text(cvs_background, MAX_X - TextW - LineW - 2, Y0 + dy - i * LineStep - 7,
                        TextW, &label, text);
  }

  lv_canvas_draw_rect(cvs_background, MAX_X - SIDE_PANEL_W, Y0 - 25, SIDE_PANEL_W - LineW, 50,
                      &rect);
...
void draw_alt_label(float altitude, uint8_t is_alt_valid, gui_unit_alt_t unit) {
  ...
  sprintf(text, "%02d\n%02d\n%02d\n%02d\n%02d", abs((Tck + 2 * step) % 100),
          abs((Tck + step) % 100), abs((Tck) % 100), abs((Tck - step) % 100),
          abs((Tck - 2 * step) % 100));
  label.align = LV_TEXT_ALIGN_CENTER;
  label.font = &lv_font_sans14;
...
void draw_vert_speed(float vert_speed, uint8_t is_vspeed_valid) {
  ...
  label.align = LV_TEXT_ALIGN_CENTER;
  label.font = &lv_font_sans16;
  lv_canvas_draw_rect(cvs_speed_v, (PANEL_VSPD_W - TextW) / 2, Y0 - TextH / 2 + 1, TextW, TextH,
                      &rect);
...
void draw_speed_scale(float speed, const uint8_t is_speed_valid) {
  ...
  for (int i = -4; i <= 3; i++) {
    ...
    lv_canvas_draw_line(cvs_background, line_maj, 2, &line);
    lv_point_t line_min[] = {{0, Y0 + dy - i * LineStep + LineStep / 2},
                             {LineW / 2, Y0 + dy - i * LineStep + LineStep / 2}};
    lv_canvas_draw_line(cvs_background, line_min, 2, &line);
...
void draw_speed_label(float speed, uint8_t is_speed_valid) {
  ...
  sprintf(text, "%1d\n%1d\n%1d\n%1d\n%1d", abs((Tck + 2 * step) % 10), abs((Tck + step) % 10),
          abs((Tck) % 10), abs((Tck - step) % 10), abs((Tck - 2 * step) % 10));
...
//------------------------------------------------------------------------------
void clear_message(const char *message) { lv_obj_add_flag(cvs_msg_box, LV_OBJ_FLAG_HIDDEN); }
//------------------------------------------------------------------------------
